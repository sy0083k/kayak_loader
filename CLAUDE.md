# CLAUDE.md

This file provides guidance to Claude Code (claude.ai/code) when working with code in this repository.



After completing any task that modifies repository files, suggest an appropriate git commit title in the final response.

Repository file changes include both code changes and documentation changes.



## Environment

* Python 3.12 virtual environment at `.venv/`
* Activate with: `source .venv/bin/activate`

## Project Status

This project is in early setup — no source files, dependencies, or test infrastructure exist yet. When they are added, update this file with:

* How to install dependencies
* How to run the project
* How to run tests (all and single)
* Architecture overview

